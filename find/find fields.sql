begin
  ...
  --Applet SS + Toggle Field
  for c in (select ap.name applet, ap.buscomp_name, ap.srchspec srchspec, tg.auto_tog_fld_name tg_fld
              from siebel.s_applet ap
             full outer join siebel.s_applet_toggle tg
                on ap.row_id = tg.applet_id
             inner join siebel.s_repository rep
                on ap.repository_id = rep.row_id
                and tg.repository_id = rep.row_id
             where rep.name = p_repository
               and (ap.name like p_object_name or ap.buscomp_name = p_object_name)
               and (ap.srchspec like p_text_to_search
                or tg.auto_tog_fld_name = l_txt)
             order by 1, 2) loop
      dbms_output.put_line('Applet: ' || c.applet || ', Search Spec: ' ||
                           c.srchspec || ', Toggle Field: ' || c.tg_fld);
      if p_print_context then
        printlines(p_long => c.srchspec);
        printlines(p_long => c.tg_fld);
      end if;
  end loop;
  --Applet Control
  for c in (select ap.name applet, ap.buscomp_name, ctrl.field_name field_name, ctrl.rowid
              from siebel.s_applet ap
             inner join siebel.s_control ctrl
                on ap.row_id = ctrl.applet_id
             inner join siebel.s_repository rep
                on ap.repository_id = rep.row_id
               and ctrl.repository_id = rep.row_id
             where rep.name = p_repository
               and (ap.name like p_object_name or ap.buscomp_name = p_object_name)
               and ctrl.field_name = l_txt
             order by 1, 2) loop
      dbms_output.put_line('Applet: ' || c.applet || ', Contol Field: ' ||
                           c.field_name);
      if p_print_context then
        printlines(p_long => c.field_name);
      end if;
  end loop;
--Applet List Column
  for c in (select ap.name applet, ap.buscomp_name, list_col.field_name field_name
              from siebel.s_applet ap
             inner join siebel.s_list list
                on ap.row_id = list.applet_id
             inner join siebel.s_list_column list_col
                on list.row_id = list_col.list_id
             inner join siebel.s_repository rep
                on ap.repository_id = rep.row_id
               and list.repository_id = rep.row_id
               and list_col.repository_id = rep.row_id
             where rep.name = p_repository
               and (ap.name like p_object_name or ap.buscomp_name = p_object_name)
               and list_col.field_name = l_txt
             order by 1, 2) loop
      dbms_output.put_line('Applet: ' || c.applet || ', List Field: ' ||
                           c.field_name);
      if p_print_context then
        printlines(p_long => c.field_name);
      end if;
  ...
  --Applet Script
  for c in (select ap.name applet, ap.buscomp_name, aps.name method, aps.script, aps.rowid
              from siebel.s_applet ap
  ...
  for c in (select ap.name applet, ap.buscomp_name, up.name user_prop, up.value
              from siebel.s_applet ap
  ...
  --BC srchspec + sortspec
  for c in (select bc.name buscomp, bc.srchspec srchspec, bc.sortspec sortspec
              from siebel.s_buscomp bc
             inner join siebel.s_repository rep
                on bc.repository_id = rep.row_id
             where rep.name = p_repository
               and bc.name = p_object_name
               and (bc.srchspec like p_text_to_search 
                or bc.sortspec like p_text_to_search)
             order by 1, 2) loop
    dbms_output.put_line('BC: ' || c.buscomp || ', srchspec: ' ||
                         c.srchspec || ', sortspec: ' || c.sortspec);
    if p_print_context then
      printlines(p_long => c.srchspec);
      printlines(p_long => c.sortspec);
    end if;
  end loop;
  --BC Script
  ...
  --BC Calculated Field + pre/post defvalue
  for c in (select bc.name buscomp, fld.name field, fld.calcval, fld.postdefval, fld.predefval
              from siebel.s_buscomp bc
             ...
             where rep.name = p_repository
               ...
               and (fld.calcval like p_text_to_search
                or fld.postdefval like p_text_to_search
                or fld.predefval like p_text_to_search)
             order by 1, 2) loop
    dbms_output.put_line('BC: ' || c.buscomp || ', Calculated Field (or pre/post defvalue): ' ||
                         c.field);
    if p_print_context then
      printlines(p_long => c.calcval);
      printlines(p_long => c.postdefval);
      printlines(p_long => c.predefval);
    end if;
  end loop;
  --BC User Property
  ...
  --Pick List
  for c in (select pl.name pl_name, pl.srchspec srchspec, pl.sortspec sortspec
              from siebel.s_picklist pl
             inner join siebel.s_repository rep
                on pl.repository_id = rep.row_id 
             where rep.name = p_repository
               and pl.buscomp_name = p_object_name
               and (pl.srchspec like p_text_to_search
                or pl.sortspec like p_text_to_search)
             order by 1, 2) loop
    dbms_output.put_line('Pick List: ' || c.pl_name || ', srchspec: ' || c.srchspec 
      || ', sortspec: ' || c.sortspec);
    if p_print_context then
      printlines(p_long => c.srchspec);
      printlines(p_long => c.sortspec);
  ...
  for c in (select svrs.name ruleset_name, svr.name rule_name
             ...
             where svrs.status_cd = 'Active'
               ...
               and (svrs.buscomp_name like p_object_name or
                   svr.buscomp_name like p_object_name)
             order by 1, 2) loop
  ...
  for c in (select distinct swp.proc_name wf_name,
             ...
             where rep.name = p_repository
               ...
               and (sws.name like p_text_to_search or
                   ...
                   swsb.expr like p_text_to_search)) loop
    ...
    if p_print_context then
      printlines(p_long => c.swsa_val);
      printlines(p_long => c.swsb_expr);
    end if;
  end loop;
  --Task step
  for c in (select stp.TASK_NAME task_name,
         ...
         stsa.VAL like p_text_to_search)) loop
  ...
  for c in (select stp.TASK_NAME task_name,
             ...
             where rep.name = p_repository
               and stp.status_cd = 'COMPLETED'
               and stsb.EXPR_TEXT like p_text_to_search) loop
    dbms_output.put_line('Task: ' || c.task_name  ||
                         (case when c.branch_name is null then '' else
                          ', Branch: ' || c.branch_name end));
  end loop;
  --EAI Component Map Field
    for c in (select om.NAME      map_name,
      ...
      inner join siebel.S_INT_FLDMAP field
          ...
          field.DST_INT_FLD_NAME like p_text_to_search)) loop
  ...
  --EAI Component Map Argument
   for c in (select om.NAME       map_name
      ...
      left outer join siebel.S_INT_MAP_ARG arg
          ...
          arg.DISPLAY_NAME like p_text_to_search)) loop
  ...
  --List Of Values
    for c in (select  lov.TYPE    TYPE,
          ...
          lov.NAME like p_text_to_search)) loop
